document.addEventListener("DOMContentLoaded", function () {
    ...
    if (kelompokForm) {
        kelompokForm.addEventListener("submit", function (e) {
            ...
            const autoCreate = document.getElementById("auto-create-group").checked;
            ...
            // Buat tabel kelompok
            buatTabelKelompok(jumlahGrup, jumlahAnggota, autoCreate);
    ...
    // --- 3. FUNGSI UNTUK MEMBUAT TABEL KELOMPOK ---
    function buatTabelKelompok(jumlahGrup, maxAnggota, autoCreate) {
        ...
        for (let i = 0; i < jumlahGrup; i++) {
            // Generate jumlah anggota acak antara 1 sampai maxAnggota
            const jumlahAnggotaSekarang = autoCreate ? 
                Math.floor(Math.random() * maxAnggota) + 1 : 
                0;
            
            const namaKelompok = `Kelompok ${kelompokHuruf[i]}`;
            
            // Gunakan data contoh jika tersedia, atau buat data baru
            const anggota = kelompokDataContoh[namaKelompok] ? 
                kelompokDataContoh[namaKelompok].map(member => member.name) : 
                (autoCreate ? generateAnggota(jumlahAnggotaSekarang) : []);
    ...
    // --- 4. FUNGSI UNTUK GENERATE ANGGOTA OTOMATIS ---
...
// Export fungsi agar bisa diakses dari onclick
window.hapusAnggota = function(button) {
    // Panggil fungsi yang ada dalam scope DOMContentLoaded
    const event = new Event('hapusAnggotaRequired');
